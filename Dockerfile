...
RUN apt-get update && \
    ...
    apt-get install -y ros-$ROS_DISTRO-tf-transformations && \
    apt-get install -y ros-$ROS_DISTRO-ament-lint python3-autopep8 && \
    apt-get install -y ros-$ROS_DISTRO-launch-testing ros-$ROS_DISTRO-launch-testing-ament-cmake ros-$ROS_DISTRO-launch-testing-ros && \
    apt-get install -y nano && \
    apt-get install -y htop && \
    apt-get install -y python3-bloom python3-rosdep fakeroot && \
    apt-get install -y dpkg-dev debhelper && \
    apt-get install -y lcov && \
    apt-get install -y ros-$ROS_DISTRO-control-msgs && \
    ...
    apt-get install -y ros-$ROS_DISTRO-ros-ign-gazebo && \
    apt-get install -y ros-$ROS_DISTRO-ros2controlcli && \
    apt-get install -y ros-$ROS_DISTRO-xacro && \
    apt-get install -y ros-$ROS_DISTRO-effort-controllers && \
    apt-get install -y ros-$ROS_DISTRO-imu-sensor-broadcaster && \
    apt-get install -y ros-$ROS_DISTRO-ros-gz-bridge && \
    apt-get install -y ros-$ROS_DISTRO-velocity-controllers && \
    apt-get install -y ros-$ROS_DISTRO-diff-drive-controller && \
    apt-get install -y ros-$ROS_DISTRO-tricycle-controller && \
    apt-get install -y ros-$ROS_DISTRO-ackermann-steering-controller && \
...
RUN pip3 install -r requirements.txt && \
    ...
    ARG WORKSPACE=/ws/ros2_ign_p2p_controllers
    RUN echo "source /opt/ros/$ROS_DISTRO/setup.bash" >> /home/$USERNAME/.bashrc && \
    mkdir -p ${WORKSPACE}/src && cd ${WORKSPACE} &&\
    git clone -b feature/forklift https://github.com/ahmadabouelainein/ros2_ign_p2p_controllers && \
    git clone -b ${ROS_DISTRO} https://github.com/ros-controls/gz_ros2_control.git && \
